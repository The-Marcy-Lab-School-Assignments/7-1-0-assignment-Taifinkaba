...
**Functionality**
- [Done] On load of the page, a user see a list of pokemon cards displaying each pokemon's name, front sprite, and HP level.
- [Done] A user can click on a pokemon card to toggle seeing its front sprite or back sprite.
- [Done] A user can use the search bar to filter pokemon by name.
- [Done] A user can fill out and submit the form to create a new pokemon. This will display the new pokemon on the page and the new pokemon data should persist, even after the page is refreshed. This means you'll have to make a POST request to our JSON Server API!
- [U Funny] Bonus: A user can additionally filter pokemon by HP

**React Fundamentals**
- [Done] `useState` is used to manage state. This is done in the Context Provider file at least once.
- [Done] `useEffect` is used to perform an asynchronous fetch call. This is done in the Context Provider file at least once.
- [Done] The form must be a controlled component. The `value` prop of the `input` elements should be set to a state value and `onChange` should update that state value.
- [Done] The project uses React Context.
- [Done] The project uses `useContext` at least once
